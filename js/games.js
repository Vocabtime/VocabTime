...
const AudioFX = {
  ...
  playWrong(targetElement = null) {
    try {
      this.init();
      if (typeof navigator !== 'undefined' && navigator.vibrate) {
        try { navigator.vibrate([80, 50, 80]); } catch (ve) {}
      }
      if (targetElement) {
...
function renderQuizItem() {
  if (currentQuizIdx >= quizQuestions.length) {
    ...
    document.getElementById('quizOptionsContainer').innerHTML = is100Percent ? `
      <div class="bg-[#fff0f3] border-2 border-rose-200 rounded-3xl p-6 sm:p-8 text-center my-4 shadow-md animate-fade-in">
        <div class="w-14 h-14 rounded-full bg-[#ff3b68] text-white flex items-center justify-center text-2xl font-black mx-auto mb-4 shadow-sm">
          <i class="fa-solid fa-check"></i>
        </div>
        <h3 class="font-black text-xl sm:text-2xl text-[#ff3b68] tracking-tight mb-2">Tuyệt vời! Hoàn thành 100%!</h3>
        <p class="text-xs sm:text-sm font-bold text-slate-600 mb-6">Bạn đã trả lời chính xác <span class="font-black text-slate-800">${quizScore}/${quizQuestions.length}</span> câu hỏi</p>
        <button onclick="startQuizRound()" class="px-8 py-3.5 rounded-full bg-[#ff3b68] hover:bg-rose-600 text-white font-black text-xs sm:text-sm uppercase tracking-wider shadow-md hover:shadow-lg transition active:scale-95 cursor-pointer">
          CHƠI VÁN KHÁC
        </button>
      </div>
    ` : `
      <div class="glass-card rounded-3xl p-6 sm:p-8 border-2 border-vtPurple border-b-4 border-b-vtPurpleDark text-center shadow-lg">
        <h3 class="font-black text-2xl text-vtPurpleDark">Bạn đạt được ${quizScore}/${quizQuestions.length} điểm!</h3>
        <p class="text-xs font-bold text-slate-500 mt-1 mb-5">Các câu hỏi và lựa chọn được đảo ngẫu nhiên sau mỗi lượt.</p>
        <button onclick="startQuizRound()" class="w-full py-4 rounded-2xl btn-vt-purple text-white font-black text-sm uppercase tracking-wider shadow-md cursor-pointer">
...
function handleMatchSelect(btn, card, index) {
  ...
  // Hủy chọn (deselect) nếu người dùng bấm lại lần 2 vào ô đang được chọn
  if (matchFirst && matchFirst.index === index) {
    btn.className = "h-20 p-2.5 rounded-2xl bg-white border-2 border-vtGrayBorder border-b-4 border-b-slate-200 text-slate-800 font-extrabold text-xs sm:text-sm text-center flex items-center justify-center transition active:translate-y-1 cursor-pointer";
    matchFirst = null;
    return;
  }
  ...
  if (!matchFirst) {
  ...
  } else if (matchFirst.index !== index) {
    ...
    if (matchFirst.card.id === matchSecond.card.id && matchFirst.card.type !== matchSecond.card.type) {
      ...
      AudioFX.playWrong(matchFirst.btn);
      AudioFX.playWrong(matchSecond.btn);
      setTimeout(() => {
        if (matchFirst && matchFirst.btn) {
        ...
        if (matchSecond && matchSecond.btn) {
