...
function loadCloudStateIntoMemory(data) {
  ...
  // 7. Apply Settings & Theme from Cloud
  if (data.themeId && typeof applyTheme === 'function') {
    applyTheme(data.themeId);
  }
  if (data.fontSize && typeof applyFontSize === 'function') {
    applyFontSize(data.fontSize);
  }
  if (typeof data.glassOpacity === 'number' && typeof applyGlassOpacity === 'function') {
...
function saveUserProgressToCloud(immediate = false) {
  ...
  const doSync = () => {
    ...
    const payload = {
      ...
      // UI Customization Sync
      themeId: typeof currentThemeId !== 'undefined' ? currentThemeId : 'purple',
      fontSize: typeof currentFontSizeId !== 'undefined' ? currentFontSizeId : 'md',
